...
object Weaver {
  ...
  def normalizedNaiveGlobalWeaving(aspects: List[Aspect], exprs: List[expr]) = {
    normalize(naiveGlobalWeaving(aspects, exprs))
  }
  ...
  def NormalizedGlobalWeaving(aspects: List[Aspect], exprs: List[expr]) = {
    normalize(GlobalWeaving(aspects, exprs))
  }
   ...
   */
  def normalize(exprs: List[expr]): List[expr] = {
    val regEx = """^x_[0-9]+$""".r
    val numRegEx = """[0-9]+$""".r
    ...
    val upper = (regular map (x => numRegEx.findFirstIn(x) match {
      case Some(nS) => nS.toInt
      case None => throw new Exception("The variable " + x + "does not match")
    })).max + 1
    ...
    replacements.foldRight(exprs)((repl: (String, String), es: List[expr]) => {
      repl match {
